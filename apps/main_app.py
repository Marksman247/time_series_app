# ==============================================
# 📊 Advanced Time Series Analysis App
# Built by MAX
# ==============================================
...
st.caption("Built by MAX")

# Upload CSV file
uploaded_file = st.file_uploader("📤 Upload your CSV file", type=["csv"])

# Run app only if a file is uploaded
if uploaded_file is not None:
    try:
        ...
        # Column selection for Date and Value
        date_col = st.selectbox("Select Date/Time column", df.columns)
        value_col = st.selectbox("Select Value column", df.columns)

        # Parse dates, clean and sort data
        ...
        # Check stationarity and allow differencing if necessary
        stationary = adf_result[1] < 0.05
        if stationary:
            st.success("✅ The time series is stationary (reject H0).")
            d = 0
        else:
            st.warning("⚠️ The time series is non-stationary (fail to reject H0).")
        ...
        # 📑 ARIMA model options in sidebar
        st.sidebar.header("⚙️ ARIMA Model Options")
        ...
        if not auto_arima:
            ...
            p = q = None  # will be chosen by auto_arima

        forecast_period = st.number_input("Forecast periods", min_value=1, max_value=60, value=10)

        # Model fitting and forecasting
        with st.spinner("Fitting ARIMA model..."):
            try:
                if auto_arima:
                    st.info("Running Auto ARIMA to find best parameters. This may take a moment...")
                ...
                fig.add_trace(go.Scatter(x=forecast_df.index, y=forecast_df['mean'], mode='lines', name='Forecast'))
                ...
                # 📊 Forecast Error for manual models
                if not auto_arima:
